...
Dit hoofdstuk biedt een analyse van de huidige situatie in de mainframe-industrie, bestaande competentiemodellen en opleidingsmethoden. Het bouwt voort op de
...
work in de volgende hoofdstukken. Hierbij wordt verwezen naar relevante literatuur om de stand van zaken te schetsen.
...
\item \textbf{Overige sectoren:} Ook in sectoren zoals energie, transport en productie (bijvoorbeeld, Arcelor Mittal in Zelzate, Tata Steel in IJmuiden) worden mainframes ingezet voor grootschalige dataverwerking en kritieke operationele systemen.
...
EBCDIC (Extended Binary Coded Decimal Interchange Code) is de traditionele tekencodering binnen z/OS-datasets. In tegenstelling tot de wereldwijd dominante ASCII-standaard gebruiken klassieke mainframeomgevingen EBCDIC voor de representatie van karakters \autocite{IBM_zOS_Concepts}. Beide zijn 8-bit tekensets, maar verschillen in binaire toewijzing en sorteervolgorde, wat impact heeft op tekstverwerking en gegevensuitwisseling. Bij communicatie tussen ASCII- en EBCDIC-gebaseerde systemen is correcte conversie essentieel; bij binaire overdracht moet expliciet een binaire transfer worden gekozen om ongewenste karakterconversie te vermijden.
...
Een address space in z/OS is het bereik van virtuele adressen dat aan een gebruiker, job of subsystem wordt toegewezen. Elke address space is logisch geïsoleerd, waardoor programma’s en data van elkaar gescheiden blijven. Binnen één address space kunnen meerdere taken gelijktijdig worden uitgevoerd, wat multiprogramming mogelijk maakt \autocite{IBM_zOS_Concepts}. Fouten blijven doorgaans beperkt tot de betrokken address space, wat de stabiliteit en beveiliging van het systeem verhoogt.
...
\subsection{\IfLanguageName{dutch}{Wat zijn de taken van een system administrator en system programmeur volgens IBM}{What is the role of a system administrator in a z/OS context}}
...
\subsection{De evoluerende rollen van SysAdmin en SysProg}
...
\subsection{Conclusie rond de uitdagingen}
...
Daarom neemt dit onderzoek deze problematiek als uitgangspunt en vertaalt deze naar concrete competentievereisten
binnen het voorgestelde framework.
...
Binnen dit onderzoek wordt CBE daarom niet enkel beschouwd als theoretisch referentiekader, maar als praktische methodologie voor het structureren van het competentieframework. De leerdoelen, oefeningen en evaluatiecriteria worden expliciet afgeleid van de gedefinieerde competenties.

\subsection{Naar een geïntegreerd competentieraamwerk}

Een effectief competentieraamwerk voor SysAdmin/SysProg (Level 1 en Level 2) binnen een z/OS-omgeving moet rekening houden met de unieke kenmerken van mainframe-technologie, de specifieke vereisten van de industrie en de pedagogische principes die effectief leren ondersteunen.
...
\section{SysAdmin/SysProg en competentieontwikkeling}
\label{sec:sysadmin-comp-ontwkile}

System Administrators en System Progremmeurs spelen een cruciale rol binnen z/OS door het waarborgen van stabiliteit, beveiliging en continuïteit van kritieke systemen. Organisaties zijn sterk afhankelijk van hun expertise voor het dagelijks beheer en de bescherming van deze infrastructuur.
...
Dit onderzoek bouwt verder op deze vaststellingen en vult de bestaande literatuur aan door deze competenties niet enkel te beschrijven, maar ook te structureren in een gelaagd competentieframework. Hierdoor worden abstracte rolverwachtingen vertaald naar concrete en toetsbare leerdoelen voor Niveau 1 en Niveau 2 profielen.
...
\subsection{Integratie van educatieve theorieën in competentieontwikkeling voor z/OS SysAdmins/SysProgs}
...
Ervaringsgericht leren benadrukt het belang van leren via concrete uitvoering, reflectie en her-toepassing \autocite{kolb2015experiential}. Voor z/OS SysAdmin/SysProg's impliceert dit een leertraject waarin technische handelingen in een labomgeving worden uitgevoerd, gevolgd door feedback en koppeling aan onderliggende systeemprincipes. 
...
\subsubsection{Conclusie: Overname van de KSA-structuur uit bestaande raamwerken}

De besproken model toont aan dat effectieve competentieontwikkeling verder gaat dan louter technische kennis, en eveneens vaardigheden en attitudes omvat.

Daarom hanteert dit onderzoek de KSA-structuur (Knowledge, Skills, Attitude) als uitgangspunt voor het competentieframework. Deze structuur wordt vervolgens verder geconcretiseerd en vertaald naar specifieke z/OS-taken en observeerbare, meetbare leeruitkomsten binnen de context van System Administration en System Programming.
...
Het e-CF beschrijft competenties op vijf niveaus (e-1 tot e-5), waarbij het niveau stijgt naarmate autonomie, complexiteit en impact toenemen. Deze structuur maakt het mogelijk om ICT-profielen, waaronder dat van SysAdmin en SysProg, objectief te positioneren \autocite{eCF_framework}.
...
\subsubsection{Relevante e-CF competenties voor System Administrators en System Programmeurs}
Binnen het e-CF kunnen volgende competenties rechtstreeks gelinkt worden:
\begin{itemize}
...
Daarom is er nood aan een competentie framework voor z/OS System Administrators en System Programmeurs dat 
\begin{itemize}
	\item Een gedetailleerde set vaardigheden biedt voor z/OS SysAdmin/SysProg, inclusief JCL, SDSF, REXX, RACF, subsystemen (basiskennis van CICS,IMS en Db2) en z/OS architectuur met niveauprogressie (Level 1 en Level 2).
